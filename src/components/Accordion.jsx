            ...
            {sections.map((section, index) => (
                <div key={index} className='rounded-lg mb-4 overflow-hidden'>
                    ...
                    >
                        ...
                        <svg
                            className={`w-5 h-5 transition-transform transform fill-mainWhite ${
                    ...
                    {section.sessions.map((session, sessionIndex) => (
                        <div
                            key={sessionIndex}
                            className={`p-4 bg-secondaryColor text-mainWhite flex items-center justify-between border-t border-primaryColor ${
                                expandedIndex === index ? 'block' : 'hidden'
                            } transition-all duration-500`}
                        >
                            <div className='flex items-center justify-center'>
                                <div className='w-8 h-8 rounded-full border border-mainWhite flex items-center justify-center'>
                                    <p>{session.id}</p>
                                </div>
                            ...
                            <div className='text-mainWhite'>{session.time}</div>
                        </div>
                    ))}
                </div>
            ))}
